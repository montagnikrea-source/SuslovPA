/* SecureShell: sandboxed algorithm runner with GuardianNet controller */
(function(global){
  'use strict';
  ...
  class GuardianNet {
    ...
    decide(metrics){
      ...
      // Простая политика: если уверенность и стабильность упали, замедляемся и/или фризим обучение
  ...
  class SecureShell {
    ...
    init(){
      return new Promise((resolve,reject)=>{
        ...
        const onMsg = (e)=>{
          if(!e || !e.data) return;
          const { type, id, ok, result, error } = e.data;
    ...
    _onMessage(e){
      ...
      if(d.type === 'rpc:res'){
        ...
        if(p){
          ...
          d.ok ? p.resolve(d.result) : p.reject(new Error(d.error||'error'));
        }
        return;
      }
      if(d.type === 'telemetry'){
        try{ this.onTelemetry(d.payload); }catch(_){ }
        // Управление через GuardianNet
  ...
  global.SecureShell = SecureShell;
})(window);
